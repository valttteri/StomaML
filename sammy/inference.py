...
def _resize_binary_mask(binary_mask: np.ndarray, width: int, height: int):
...
def _build_instance(binary_mask: np.ndarray, class_id: int, class_name: str, confidence: float):
...
def _build_stomata_metadata(binary_mask: np.ndarray, instance_id: int, class_id: int, confidence: float, um_per_px: float):
    x1, y1, x2, y2 = mask_to_bbox(binary_mask)
    ys, xs = np.where(binary_mask)
    ellipse_length_px, ellipse_width_px = get_ellipse_axes(binary_mask)
    pixel_area = int(binary_mask.sum())
...
def process_yolo_result(result, um_per_px: float):
    ...
    for mask_float, conf, cls in zip(masks_float, confidences, class_ids):
        ...
        binary = _resize_binary_mask(binary, img_w, img_h)

        if int(binary.sum()) == 0:
            continue

        class_counter[cls_name] += 1
        instance = _build_instance(binary, cls_id, cls_name, float(conf))
    ...
    for binary, cls_id, conf in stomata_candidates:
        ...
        if overlap_fraction >= COUNTABLE_OVERLAP_THRESHOLD:
            meta = _build_stomata_metadata(binary, kept_index, cls_id, conf, um_per_px)
